...
/*   Updated: 2025/08/20 11:46:57 by zslowian         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_cub3d *data)
{
	...
	while (++i < 3)
	{
		if (!color_values[i])
			ft_error(COLOR,
				"ft_store_rgb - insufficient RGB color values provided", data);
		color = ft_atoi((const char *)color_values[i]);
		if (color < 0 || color > 255)
			ft_error(COLOR,
				"ft_store_rgb - RGB color values must be between 0 and 255",
				data);
