...
float find_vertical_intersection_x(float player_x, int is_ray_facing_right)
{
    ...
    //else - if it's facing left
        ...
        intersection_x = floorf(player_x / GRID_SIZE) * GRID_SIZE - 1; //left edge of the players current grid, the -1 puts the point inside the grid cell
...
float look_for_wall_on_vertical(float ray_angle, float player_x, float first_x_intersection_line)
{
    is_ray_facing_right = is_ray_facing_right(ray_angle);

    //where to take first_x_intersection_line from?
    float first_vertical_x = find_vertical_intersection_x(player_x, is_ray_facing_right);
    float first_vertical_y = find_vertical_intersection_y(player_x, player_y, first_x_intersection_line, ray_angle_rad);
