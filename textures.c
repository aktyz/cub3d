...
unsigned int	get_texture_pixel_color(t_texture_data *texture, int x, int y)
{
	...
	if (x < 0 || x >= texture->width || y < 0 || y >= texture->height)
		return (0x0);
	pixel = texture->img.pix_ptr + (y * texture->img.line_len + x
			* (texture->img.bpp / 8));
...
{
	load_single_texture(data, &data->textures_data[NO],
		data->textures->no_texture);
	load_single_texture(data, &data->textures_data[SO],
		data->textures->so_texture);
	load_single_texture(data, &data->textures_data[WE],
		data->textures->we_texture);
	load_single_texture(data, &data->textures_data[EA],
