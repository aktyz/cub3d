...
/*   Updated: 2025/08/21 15:41:21 by zslowian         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool					ft_add_map_token(int *i, char *line, t_token *last, t_cub3d *data);
...
char					**ft_copy_map(t_cub3d *data, int map_rows,
							int map_cols);
void					ft_free_map_copy(char **map_copy, int map_rows);
void					ft_switch_remaining_zeroes(char **map_copy,
							t_cub3d *data);

// ERROR HANDLING
void					ft_error(t_cub3d_errors nb, char *ft_name,
							t_cub3d *data);
...
// check_horizontal.c
float					find_first_horizontal_intersection_y(t_player *player,
							float ray_angle);
float					find_first_horizontal_intersection_x(t_player *player,
							float ray_angle);
...
// check_vertical.c
float					find_first_vertical_intersection_x(t_player *player,
							float ray_angle);
float					find_first_vertical_intersection_y(t_player *player,
							float ray_angle);
float					find_vertical_step_x(float ray_angle);
float					find_vertical_step_y(float ray_angle);

// cast_rays_utils.c
int						is_wall_on_grid(float intersection_x,
							float intersection_y, t_cub3d *data);
float					distance_to_the_wall(t_player *player, int column,
							float intersection_x, float intersection_y);
void					find_horizontal_intersection(t_player *player,
							t_cub3d *data, float ray_angle,
							t_intersection *intersection);
void					find_vertical_intersection(t_player *player,
							t_cub3d *data, float ray_angle,
							t_intersection *intersection);
void					find_wall(t_player *player, t_cub3d *data,
							float ray_angle, t_intersection *intersection);

// cast_rays.c
float					find_and_set_wall_data(t_cub3d *data, int column,
							float ray_angle, t_intersection *intersection);
...
unsigned int			get_texture_pixel_color(t_texture_data *texture, int x,
							int y);
void					load_single_texture(t_cub3d *data,
							t_texture_data *tex_data, char *path);
